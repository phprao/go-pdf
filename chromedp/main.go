...
func ProcessTarGzFile(filename string) error {
	...
	for k, html := range htmls {
		...
		if err := HtmlToPdf(dstdir, html, outputPdf); err != nil {
			return err
		}
	...
	if err := unipdf.MergePdf(htmls, outputPdfName); err != nil {
		fmt.Println(err)
		return err
	}
...
func (s SortStringSliceIncrement) Less(i, j int) bool {
	...
	// Cover 排在第一位
	stri := s[i][strings.LastIndex(s[i], "/"):strings.LastIndex(s[i], ".")]
	if strings.Contains(stri, "Cover") {
		return true
	}

	strj := s[j][strings.LastIndex(s[j], "/"):strings.LastIndex(s[j], ".")]
